...
router.post("/create", upload.array('images', 10), (req, res) => {
    let { product_name, product_type, product_additional } = req.body
    if (!req.files.length || product_name === "" || product_type === "" || product_additional === "") {
        res.send(400, {
        ...
        database.query(`INSERT INTO products (product_name, product_type, product_additional , product_image_url ) VALUES ('${product_name}', '${product_type}', '${product_additional}', '${file_name}')`, (err, result) => {
            if (err) throw err
...
router.put("/update", upload.array('images', 10), (req, res) => {
    let { item_id, product_name, product_type, product_additional } = req.body
    if (product_name === "" || product_type === "" || product_additional === "") {
        res.send(400, {
            message: "Iltimos ma'lumotlarni to'liq kiriting!"
        })
    } else {
        database.query(`UPDATE products SET (product_name, product_type, product_additional) = ('${product_name}', '${product_type}', '${product_additional}') WHERE id = ${item_id}`, (err, result) => {
            if (err) throw err
